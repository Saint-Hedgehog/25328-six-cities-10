...
import { AppRoute, LinkParameter, LogoutText, Timer } from '../../../../const';
...
const HeaderUser: React.FC = () => {
  ...
  const isFavoritePath = currentPath === AppRoute.Favorites;
  const linkParameter = isFavoritePath ? LinkParameter.Disabled : LinkParameter.Active;
  ...
  const linkUserNameClass = classNames('header__nav-link header__nav-link--profile', { 'link__disabled': isFavoritePath });
  const linkSingoutClass = classNames('header__nav-link', { 'link__disabled': isLogoutError || isLogoutProcessing });
  const spanSignoutClass = classNames('header__signout', { 'header__signout__error horizontal-shake': isLogoutError });
  ...
  return (
    <Fragment>
      <li className="header__nav-item user">
        <Link to={AppRoute.Favorites} style={{ pointerEvents: linkParameter }} className={linkUserNameClass}>
